...
const Profile = () => {
  ...
  useEffect(() => {
    if (students) {
      setProfileData({
        ...
        // dob: students?.dob ? new Date(students.dob).toISOString() : '',
  ...
  const handlegetUserData = async () => {
    try {
      const response = await axios.get(`http://10.0.2.2:5000/principal/information/${user?.id}`);
  ...
  const updateProfile = async () => {
    ...
    try {
      const response = await axios.put(`http://10.0.2.2:5000/principal/update`, {
        _id: students?._id,
  ...
  const selectProfileImage = async () => {
    ImagePicker.openPicker({
      ...
      if (!response.didCancel) {
        ...
        await axios.post(`http://10.0.2.2:5000/principal/upload/${students._id}`, formData, {
          headers: {
  ...
  const selectBackgroundImage = async (item) => {
    ...
    ImagePicker.openPicker({
      ...
      if (!response.didCancel) {
        ...
        // If the user cancels the image picker, clear the galleryImage state
        setGalleryImage('');
      }
    }).catch(error => {
      console.log('ImagePicker Error: ', error);
  ...
  const removeBackgroundImage = async () => {
    try {
      ...
      formData.append('key', 'background');
      formData.append('image', '');
      const uploadResponse = await axios.post(`http://10.0.2.2:5000/principal/upload/${students._id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <Card style={styles.card}>
          <View style={styles.galleryContainer}>
            <Image
              // source={{ uri: galleryImage ? `http://10.0.2.2:5000/${galleryImage || students?.background}` : 'https://via.placeholder.com/300' }}
              // source={{ uri: galleryImage ? `http://10.0.2.2:5000/${galleryImage}` : defaultGalleryImage }}
              source={{ uri: galleryImage ? `http://10.0.2.2:5000/${galleryImage || students?.background}` : defaultGalleryImage }}
          ...
          <View style={styles.profileSection}>
            <View style={styles.imageContainer}>
              <TouchableOpacity onPress={selectProfileImage} style={styles.imagePicker}>
                <Image
                  // source={{ uri: `http://10.0.2.2:5000/${profileImage || students?.image} ` }}
                  source={{ uri: profileImage ? `http://10.0.2.2:5000/${profileImage || students?.background}` : defaultProfileImage }}
